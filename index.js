...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const taskComponent = `<div class="main__list" data-id="${id}">
...
function doneTask(target) {
    ...
    if (indexTaskToDone === -1) return; // Если задачи нет в массиве, выход

    // Перемещаем задачу из списка активных в выполненные
    const doneTask = tasks.splice(indexTaskToDone, 1)[0];
    doneTasks.push(doneTask);

    // Создаём HTML для выполненной задачи
    const htmlComponent = `
    ...
    task.remove();
    resolvedTaskList.insertAdjacentHTML('beforeend', htmlComponent);
}
...
function deleteTask(target, typeTasks) {
    ...
    if (typeTasks == 'doneTasks') {
        ...
        tasks = tasks.filter(el => el.id != taskToDelete.id);
...
function createTask() {
    const taskText = createTaskInput.value // Сохранили в переменную, то что написали в форму

    // Обработчик если не написали в форму ничего
    if (!taskText) {
        return alert('Вы не написали название задачки :(');
    }
